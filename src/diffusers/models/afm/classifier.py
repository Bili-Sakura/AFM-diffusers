...
class Classifier(nn.Module):
    def __init__(
        ...
        **_,
        ...
        self.cls_embed = nn.Parameter(torch.randn([hidden_size]) * 0.02)
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        ...
        remapped = remap_transformer_state_dict(
            state_dict,
            self._legacy_kwargs,
            extra_prefixes=("cls_embed", "final_layer."),
        )
        return super().load_state_dict(remapped, strict=strict)

    def forward(self, x, t=None):
        hidden_states = self.transformer.pos_embed(x)
        cls_token = self.cls_embed.view(1, 1, -1).expand(hidden_states.shape[0], 1, -1)
        ...
        return self.final_layer(hidden_states).squeeze(1)
