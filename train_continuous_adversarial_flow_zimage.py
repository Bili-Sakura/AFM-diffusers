...
from diffusers.models.cafm.zimage.discriminator import ZImageTransformer2DModelDiscriminatorJVP
...
class ContinuousAdversarialFlowTrainer(Entrypoint, PersistenceMixin, WriterMixin):
    ...
    def configure_models(self):
        ...
        self.configure_dis_model(device="cpu")
    ...
    def configure_dis_model(self, device=get_device()):
        self.dis = ZImageTransformer2DModelDiscriminatorJVP().to(device)
        self.dis.enable_gradient_checkpointing()
        self.dis.set_attention_backend("_native_math")
        self.dis.load_state_dict(self.dit.state_dict(), strict=False)
    ...
    def configure_dit_fsdp_model(self):
        ...
        self.dis = self.wrap_dit_with_fsdp(self.dis)
    ...
    def configure_optimizers(self):
        ...
        dis_lr = optimizer_config.pop("dis_lr")
        weight_decay = optimizer_config.pop("weight_decay", 0.01)
        betas = optimizer_config.pop("betas", (0.0, 0.95))

        self.gen_optimizer = AdamW(params=self.dit.parameters(), lr=gen_lr, weight_decay=weight_decay, betas=betas, fused=True)
    ...
    def training_step(
        ...
        *,
        ...
        with (
            torch.no_grad() if dis_step else nullcontext(),
        ):
            velocity_pred = self.dit(latents_noised, timesteps, text_embeds).sample

        self.dis.requires_grad_(dis_step)

        if dis_step:
            cp_scale = self.config.adversarial.get("cp_scale", 0.001)

            out, logits_fake, logits_real = self.dis(
                latents_noised,
                timesteps,
                text_embeds,
                velocity_pred,
            ...
            gen_ot_loss = torch.stack([v.square().mean().mul(ot_scale) for v in velocity_pred]).mean()
        ...
        visual_dict = {
            "latents_pred": [x_t + (1 - t) * v.detach() for x_t, v, t in zip(latents_noised, velocity_pred, timesteps)],
        }
